...
app.post('/friends/:uid/notes', ensureLoggedIn, function(req, res, next){
  storage.insertNotes(req.cookies.twitter_id, req.params.uid, req.body.content, function(err, notes){
    if (err) {
        return res.status(500).send(err)
    }
    res.send(notes)
...
app.put('/friends/:uid/notes/:noteid', ensureLoggedIn, function(req, res, next){
  ...
  storage.updateNote(req.params.noteid, req.cookies.twitter_id, req.body.content,  function(err, note){
    ...
    res.send({ _id: note._id, content: note.content})
  })
})

app.delete('/friends/:uid/notes/:noteid', ensureLoggedIn, function(req, res, next){
  storage.deleteNote(req.cookies.twitter_id, req.params.uid, req.body.content, function(err, notes){
    if (err) {
      return res.status(500).send(err)
    }
    res.sendStatus(200)
